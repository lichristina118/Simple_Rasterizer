...
const int SceneDemoApp::windowWidth = 800;
const int SceneDemoApp::windowHeight = 600;


// Constructor runs after nanogui is initialized and the OpenGL context is current.
SceneDemoApp::SceneDemoApp()
: nanogui::Screen(Eigen::Vector2i(windowWidth, windowHeight), "Christina Li's Demo App", false),
...
bool SceneDemoApp::keyboardEvent(int key, int scancode, int action, int modifiers) {
...
bool SceneDemoApp::mouseButtonEvent(const Eigen::Vector2i &p, int button, bool down, int modifiers) {
    return Screen::mouseButtonEvent(p, button, down, modifiers) ||
           cc->mouseButtonEvent(p, button, down, modifiers);
}

bool SceneDemoApp::mouseMotionEvent(const Eigen::Vector2i &p, const Eigen::Vector2i &rel, int button, int modifiers) {
    return Screen::mouseMotionEvent(p, rel, button, modifiers) ||
           cc->mouseMotionEvent(p, rel, button, modifiers);
}

bool SceneDemoApp::scrollEvent(const Eigen::Vector2i &p, const Eigen::Vector2f &rel) {
...
void SceneDemoApp::drawContents() {
